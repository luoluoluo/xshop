...
import { getAffiliateOrders } from "@/requests/order.server";
...
export default async function Page({
  ...
  const orders = await getAffiliateOrders({
    skip,
    take,
    ...
    return res.data?.affiliateOrders;
  });
  return (
          ...
          {orders?.data?.length ? (
            <>
              <div className="flex flex-col gap-8 mt-8">
                {orders?.data?.map((v) => (
                  <OrderItem link key={v.id} order={v} affiliate></OrderItem>
                ))}
              </div>
              <Pagination
                page={page}
