...
export class WechatService {
  ...
  getOauthAccessToken = async (
    ...
    const url = `${GET_OAUTH_ACCESS_TOKEN_URL}?${new URLSearchParams(data).toString()}`;
    const res = await fetch(url);
    const resData = (await res.json()) as GetOauthAccessTokenResponse;

    if (resData.errcode) {
      this.logger.error({ url, data, resData }, 'getOauthAccessToken error');
  ...
  getUserInfo = async (
    ...
    const url = `${GET_USER_INFO_URL}?${new URLSearchParams(data).toString()}`;
    const res = await fetch(url);
  ...
  getAccessToken = async (): Promise<string> => {
    const accessToken = await this.redisService.get(ACCESS_TOKEN_CACHE_KEY);
    if (accessToken) {
      return accessToken;
    }
    const data = {
    ...
    const url = `${GET_ACCESS_TOKEN_URL}?${new URLSearchParams(data).toString()}`;
    const res = await fetch(url);
  ...
  sendTemplateMessage = async (
    ...
    const url = `${SEND_TEMPLATE_MESSAGE_URL}?access_token=${accessToken}`;
    try {
      const res = await fetch(url, {
        method: 'POST',
  ...
  sendCustomMessage = async (
    ...
    const accessToken = await this.getAccessToken();
    const url = `${SEND_CUSTOM_MESSAGE_URL}?access_token=${accessToken}`;
    try {
      const res = await fetch(url, {
        method: 'POST',
        body: JSON.stringify(data),
  ...
  getTicket = async (): Promise<string> => {
    const ticket = await this.redisService.get(TICKET_CACHE_KET);
    if (ticket) {
      return ticket;
    }
    const accessToken = await this.getAccessToken();
    const res = await fetch(
      `${GET_TICKET_URL}&access_token=${accessToken}`,
    ).then((res) => res.json() as Promise<GetTicketResponse>);
    if (res.errcode) {
