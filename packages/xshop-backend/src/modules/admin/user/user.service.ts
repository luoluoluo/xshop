...
export class UserService {
  ...
  async completeUserWechatMerchant(
    ...
    const user = await this.userRepository.findOne({
      where: { wechatMerchantId: data.wechatMerchantId },
    });
    if (!user) {
      throw new NotFoundException(`用戶ID ${data.wechatMerchantId} 未找到`);
    }
    user.wechatMerchantId = data.wechatMerchantId;
    user.wechatMerchantStatus = WechatMerchantStatus.COMPLETED;
